<div class="container-fluid d-flex justify-content-center align-items-center gap-2 p-3">
    <form class="w-50" action="./controllers/breed/update.php" method="POST">
        <input type="hidden" name="id" value="<?= $breed['id'] ?>">
        <div class="mb-3">
            <label for="petBreed" class="form-label">Pet Breed</label>
            <input type="text" name="petBreed" class="form-control" id="petBreed" value="<?= $breed['name'] ?>" required>
        </div>
        <div class="float-end">
            <a href="/admin/?view=Breeds" class="btn btn-secondary">Close</a>
            <button type="submit" class="btn btn-primary" name="update<?= $view ?>">Update</button>
        </div>
    </form>
</div>